...
const createAccount = async(createduser) => {
    ...
    const result = await fetch('http://localhost:5023/api/User/AddUsers', {
        method: "POST",
        headers: {
...
const login = async (loginUser) => {
    const result = await fetch('http://localhost:5023/api/User/Login', {
        method: "POST",
        headers: {
            "Content-Type": "application/json"
        },
...
{
    let result = await fetch(`http://localhost:5023/api/User/GetUserByUserName/${username}`)
...
{
    const result = await fetch('http://localhost:5023/api/Blog/AddBlogItems', {
        method: "POST",
        headers: {
            "Content-Type": "application/json"
        },
...
{
    const result = await fetch(`http://localhost:5023/api/${controller}/${endpoint}`, {
        method: "POST",
        headers: {
            "Content-Type": "application/json"
        },
...
{
    let result = await fetch("http://localhost:5023/api/Blog/GetBlogItem")
    let data = await result.json()
...
{
    let result = await fetch(`http://localhost:5023/api/Blog/GetItemsByUserId/${UserId}`)
    let data = await result.json()
...
{
    const result = await fetch(`http://localhost:5023/api/Blog/UpdateBlogItems`, {
        method: "POST",
        headers: {
            "Content-Type": "application/json"
        },
...
{
    let result = await fetch("http://localhost:5023/api/Blog/GetPublishedItems");
    let data = await result.json();
    return data;
}
